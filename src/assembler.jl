...
during assembly and converts it into a `SparseMatrixCSC{T}` on finalization.
"""
...
function assemble!(a::COOAssembler{T}, dofs::AbstractVector{Int}, Ke::AbstractMatrix{T}, fe::Union{AbstractVector{T}, Nothing} = nothing) where {T}
    ...
    if fe !== nothing
        # If the user passes `fe` we can be nice and extend the vector here.
...
Finalizes an assembly. Returns a sparse matrix with the assembled values and (optionally)
the assembled vector. If the assembler have not been used for vector assembly `f` is an
empty vector.
"""
